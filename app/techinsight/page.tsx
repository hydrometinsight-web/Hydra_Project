...
export default function TechInsightPage() {
  ...
  useEffect(() => {
    // Set page metadata
    document.title = 'TechInsight | HydroMetInsight'
    const metaDescription = document.querySelector('meta[name="description"]')
    if (metaDescription) {
      metaDescription.setAttribute('content', 'In-depth technical insights and analysis on hydrometallurgy processes, technologies, and innovations.')
    }
  ...
  useEffect(() => {
    setLoading(true)
    // Fetch tech insights with pagination and filters
    ...
    fetch(`/api/techinsight?${params.toString()}`)
      .then((res) => res.json())
      .then((data) => {
        if (data && !data.error) {
  ...
  const clearFilters = () => {
    ...
    setDateTo('')
    setCurrentPage(1)
    router.push('/techinsight?page=1')
  }
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <p className="text-gray-600 text-lg">
            {searchQuery || dateFrom || dateTo
              ? 'No tech insights found matching your filters.'
